function [PowerSysRes, WaterSysRes, PowerZoneRes, WaterZoneRes] = EvaluatePowerWaterWorstCaseRes(PowerSystem,WaterSystem,PowerWaterInterdependency,...
    AttackParams,OperatorParams,TerminalZone,ResEvaParams,PowerResilienceGoal,WaterResilienceGoal)
% INTRODUCTION
%   Quantifies the resilience of interdependent power每water systems under a single
%   worst-case disruption. The function identifies the attacker＊s worst-case target,
%   simulates coupled system restoration using rule-based or heuristic repair
%   strategies, and evaluates time-dependent functionality of both systems through
...
%       Each is a struct with fields .Node (1℅N) and .Edge (1℅E).
...
%         .PowerNodeAttackCost, WaterNodeAttackCost: N*1 vector specifying the cost to attack each node.(REQUIRED when AttackType='NLC')
%         .PowerEdgeAttackCost, WaterEdgeAttackCost: E*1 vector specifying the cost to attack each edge.(REQUIRED when AttackType='NLC')
%
%   OperatorParams :
...
%             .RepairCrew     : [P W] positive integers for power/water repair teams
...
%             .CritTimes      : 1℅K vector of time checkpoints (default [0 3 7 30 90])
%             .CritWeights    : 1℅K vector (normalized to sum=1 if not already)
%
%   PowerResilienceGoal, WaterResilienceGoal:
...
%
% OUTPUTS
%   PowerSysRes, WaterSysRes:
%                      scalar for 'ResLoss', 'CritTime', and 'UserGoal' 〞 the mean across simulations.
...
if nargin < 6
    error(['EvaluatePowerWaterSeismicRes: not enough input args. ', ...
        'Need PowerSystem,WaterSystem,PowerWaterInterdependency,AttackParams,OperatorParams,TerminalZone,ResEvaParams.']);
end
...
if ~isfield(ResEvaParams,'RepairCrew')
    error('ResEvaParams.RepairCrew is required and must be a 1x2 positive integer array.');
end
if ~isnumeric(ResEvaParams.RepairCrew) || numel(ResEvaParams.RepairCrew)~=2 || any(ResEvaParams.RepairCrew<=0) ||...
        any(floor(ResEvaParams.RepairCrew)~=ResEvaParams.RepairCrew)
    error('ResEvaParams.RepairCrew must be a 1x3 array of positive integers, e.g., [P W].');
end
...
switch ResEvaParams.ResMetric
    case 'UserGoal'
        ...
        if ~(nargin >= 8 && exist('PowerResilienceGoal','var') && ~isempty(PowerResilienceGoal) && ...
                exist('WaterResilienceGoal','var')   && ~isempty(WaterResilienceGoal))
            error('ResMetric=UserGoal requires PowerResilienceGoal and WaterResilienceGoal (both zone-length vectors).');
        end
        Z = numel(TerminalZone);
        sysNames = {'Power','Water'};
        goals = {PowerResilienceGoal, WaterResilienceGoal};
        for ii = 1:2
            g = goals{ii};
...
switch ResEvaParams.RepairStrategy
    case 'Rule'
        ...
        ruleFields = {'PowerRuleType','WaterRuleType'};
...
AttackStrateg = SolveWorstCaseAttackPowerWater(AttackParams,OperatorParams,PowerSystem,WaterSystem,PowerWaterInterdependency,TerminalZone);
        ...
        switch OperatorParams.PowerFunMetric
            case 'DCPF'
                [PowerResLoss, WaterResLoss, PowerSysFunsEvo, WaterSysFunsEvo, PowerRepairSeq, WaterRepairSeq, PowerZoneStateEvo, WaterZoneStateEvo]...
                    = RuleRepairPowerDCPFWaterMF(PowerSystem, WaterSystem, PowerWaterInterdependency, PowerCISComDamgScenario, WaterCISComDamgScenario, TerminalZone, ResEvaParams);
            case 'MF'
                [PowerResLoss, WaterResLoss, PowerSysFunsEvo, WaterSysFunsEvo, PowerRepairSeq, WaterRepairSeq, PowerZoneStateEvo, WaterZoneStateEvo] ...
                    = RuleRepairPowerMFWaterMF (PowerSystem, WaterSystem, PowerWaterInterdependency, PowerCISComDamgScenario, WaterCISComDamgScenario, TerminalZone, ResEvaParams);
        end
    case 'Heuristic'
        switch OperatorParams.PowerFunMetric
            case 'DCPF'
                [PowerResLoss, WaterResLoss, PowerSysFunsEvo, WaterSysFunsEvo, PowerRepairSeq, WaterRepairSeq, PowerZoneStateEvo, WaterZoneStateEvo] ...
                    = HeuristicPowerDCPFWaterMF (PowerSystem, WaterSystem, PowerWaterInterdependency, PowerCISComDamgScenario, WaterCISComDamgScenario, TerminalZone, ResEvaParams);
            case 'MF'
                [PowerResLoss, WaterResLoss, PowerSysFunsEvo, WaterSysFunsEvo, PowerRepairSeq, WaterRepairSeq, PowerZoneStateEvo, WaterZoneStateEvo] =...
                    HeuristicPowerMFWaterMF (PowerSystem, WaterSystem, PowerWaterInterdependency, PowerCISComDamgScenario, WaterCISComDamgScenario, TerminalZone, ResEvaParams);
        end
end
...
function AttackStrategy = SolveWorstCaseAttackPowerWater(AttackParams,OperatorParams,PowerSystem,WaterSystem,PowerWaterInterdependency,TerminalZone)
% INTRODUCTION:
%   Solve the worst?case attack selection for interdependent power每water systems
%   under the specified attacker model and operator modeling choice. Supports
%   location?constrained (LC) and non?location?constrained (NLC) attacker models
%   and dispatch evaluation via MF or DCPF for the power system.
%
% INPUT:
...
%       .FunMetric          : optional, default 'TotalDemand' used internally
%   PowerSystem, WaterSystem  : CIS structs (with .Node, .Edge)
%   PowerWaterInterdependency : struct describing coupling between power and water
%
% OUTPUT:
...
switch AttackParams.AttackType
    case 'LC'
        ...
        switch OperatorParams.PowerFunMetric
            case 'MF'
                AttackStrategy = LcAttackerRobPowerMFWaterMFOperator(PowerSystem, WaterSystem, PowerWaterInterdependency,...
                    ComSet, struct(), AttackParams,TerminalZone);
            case 'DCPF'
                AttackStrategy = LcAttackerRobPowerDCPFWaterMFOperator(PowerSystem, WaterSystem, PowerWaterInterdependency,...
                    ComSet, struct(), AttackParams,TerminalZone);
        end
    case 'NLC'
        % Choose worst-case identification model
        switch OperatorParams.PowerFunMetric
            case 'MF'
                AttackStrategy = NLcAttacterRobPowerMFWaterMFOperator( PowerSystem, WaterSystem, PowerWaterInterdependency,...
                    OperatorParams, AttackParams,TerminalZone);
            case 'DCPF'
                AttackStrategy = NLcAttacterRobPowerDCPFWaterMFOperator( PowerSystem, WaterSystem, PowerWaterInterdependency,...
                    OperatorParams, AttackParams,TerminalZone);
        end
end
end
...
%       .ResMetric   : 'ResLoss' | 'CritTime' | 'UserGoal'
%       .CritTimes   : 1℅K vector of time checkpoints (for 'CritTime' only)
%       .CritWeights : 1℅K vector of nonnegative weights, sum normalized to 1 (for 'CritTime' only)
%   SysFunsEvoAll     : [T℅2] (col1=time, col2=functionality LOSS)
%   ZoneStateEvoAll   : [Z℅(1+M)] (col1=ZoneID, others=zone functionality)
%   TimeAxisAll       : [M℅1] time vector matching ZoneStateEvoAll columns 2..end
%   SysResLossAll     : [1℅3] = [normLoss, realRes, targetRes]
%   TerminalZone      : 1℅Z struct array (uses .Population for 'UserGoal' weighting)
%   ResilienceGoal    : [Z℅1] zone goals (only for 'UserGoal')


%
% OUTPUTS
%   SysRes            : scalar 
%   ZoneRes           : [Z℅2] = [ZoneID, mean across simulations]
