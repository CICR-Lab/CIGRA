function [PowerRetroID, GasRetroID, PowerSysResGain, GasSysResGain, PowerZoneResGain, GasZoneResGain] = RuleRetrofitPowerGasSesimicResilience...
     (PowerSystem, GasSystem, PowerGasInterdependency, SeismicMagnitudeEpicenter, TerminalZone, params)
% INTRODUCTION
%   Perform rule-based retrofit selection for interdependent power and gas
%   systems (nodes only). The function constructs an undirected supra-graph
%   linking all three networks through intra- and inter-system edges,
%   computes node scores based on the selected rule ('degree',
...
%% Step 3: Select retrofit nodes across power¨Cgas systems
...
retroVarIn = {PowerRetroID, GasRetroID};
...
[PSysResB, GSysResB,PZoneResB,GZoneResB] = ComputeRuleBasedPowerGasResilienceLoss(PowerSystem,GasSystem,PowerGasInterdependency,...
...
[PSysResA, GSysResA,PZoneResA,GZoneResA] = ComputeRuleBasedPowerGasResilienceLoss(PowerSystem,GasSystem,PowerGasInterdependency,...
...
function [PowerSysRes, GasSysRes, PowerZoneRes, GasZoneRes] = ComputeRuleBasedPowerGasResilienceLoss(PowerSystem,GasSystem,PowerGasInterdependency,...
    PowerComDamgScenario,GasComDamgScenario, TerminalZone, params)
% INTRODUCTION
%   Computes the expected resilience loss of interdependent power and gas
%   systems under sampled component damage scenarios. For each Monte Carlo
%   sample, the function assembles system-specific damage states, invokes
%   the selected rule-based restoration model, and evaluates the
%   system-level resilience loss of each subsystem. The final outputs
%   represent the expected resilience loss of power and gas systems across
%   all simulations.
%
% INPUTS:
%   PowerSystem, GasSystem :
