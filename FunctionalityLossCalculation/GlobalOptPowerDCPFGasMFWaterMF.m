function [PowerSysFunLoss, GasSysFunLoss, WaterSysFunLoss, PowerComState, GasComState, WaterComState, PowerZoneState,  ...
    GasZoneState, WaterZoneState] = GlobalOptPowerDCPFGasMFWaterMF(PowerSystem, GasSystem, WaterSystem, PowerGasInterdependency,  ...
...
%   Max flow model is used to model both gas and water systems. Direct
%   current (DC) power flow model is used to model power system. 
%
% INPUTS:
...
Npv = E_p+Nsp+Ndp+NPdg+NPdw+N_p; Nps = NPdg+NPdw+NwdP; Npt_t = Npv+Nps;
...
idx_power_angle = E_p + Nsp + Ndp + NPdg + NPdw+ (1:N_p);
...
% (4) Reference node constraint for DC power flow
Aeq4 = zeros(1,Nv); Aeq4(idx_power_angle(params.RefNode))=1;
% (5) Power edge DC flow constraints
Aeq5 = zeros(E_p,Nv);
for e=1:E_p
    Aeq5(e,e)=1;
    if ~isDamgPowerEdge(e) && ~isDamgPowerNode(EdgePowerFrom(e)) && ~isDamgPowerNode(EdgePowerTo(e))
        Aeq5(e,idx_power_angle(EdgePowerFrom(e)))=-EdgePowerSus(e);
        Aeq5(e,idx_power_angle(EdgePowerTo(e)))=EdgePowerSus(e);
    end
end
% (6) Interdependency link flow capacity constraints
% (7) Interdependency state constraints
Aeq6 = zeros(NPdg+NPdw+NWdp,Nv);Aeq7 = zeros(NPdg+NPdw+NWdp,Nv);
for i=1:NPdg
    Aeq6(i,idx_power2gas_dem(i))=1; Aeq6(i,idx_power2gas_state(i))=-Dpg(i,3);
    Aeq7(i,idx_power2gas_state(i))=1; Aeq7(i,idx_gasInt_bin(PG_Index(i,2)))=-1;
end
for i=1:NPdw
    Aeq6(i,idx_power2water_dem(i))=1; Aeq6(i,idx_power2water_state(i))=-Dpw(i,3);
    Aeq7(i,idx_power2water_state(i))=1; Aeq7(i,idx_waterInt_bin(PW_Index(i,2)))=-1;
end
for i=1:NWdp
    Aeq6(i,idx_water2power_dem(i))=1; Aeq6(i,idx_water2power_state(i))=-Dwp(i,3);
    Aeq7(i,idx_water2power_state(i))=1; Aeq7(i,idx_powerInt_bin(WP_Index(i,2)))=-1;
end
