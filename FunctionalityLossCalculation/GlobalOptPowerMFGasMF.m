function [PowerSysFunLoss, GasSysFunLoss, PowerComState, GasComState, PowerZoneState, GasZoneState] = GlobalOptPowerMFGasMF...
...
%   modeled using a max flow model.
%
% INPUTS:
...
Aeq3 = zeros(NPdg,Nv);Aeq4 = zeros(NPdg,Nv);
for i=1:NPdg
    Aeq3(i,idx_power2gas_dem(i))=1; Aeq3(i,idx_power2gas_state(i))=-Dpg(i,3);
    Aeq4(i,idx_power2gas_state(i))=1; Aeq4(i,idx_gasInt_bin(PG_Index(i,2)))=-1;
end
% (5) Gas-to-power conversion constraints
p_nid=unique(Cgp(:,2)); 
Aeq5 = zeros(numel(p_nid),Nv);
for i=1:size(Aeq5,1)
    Aeq5(i,idx_power_gen(PowerGenIdx==p_nid(i)))=1;
    gidxs=find(Cgp(:,2)==p_nid(i));
    for j=1:length(gidxs)
        Aeq5(i,idx_gas2power_dem(gidxs))=-Cgp(gidxs(j),3);
    end
end
