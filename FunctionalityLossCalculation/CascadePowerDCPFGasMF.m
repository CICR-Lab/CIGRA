function [PowerSysFunLoss, GasSysFunLoss, PowerComState, GasComState, PowerZoneState, GasZoneState, CascadeTrace] = CascadePowerDCPFGasMF(...
...
%   interdependent power and gas systems. The operational state of each
%   system is assessed using a linear programming formulation: the gas
%   system is modeled using a max flow approach, while the power system
%   uses a direct current (DC) power flow model. Interdependency
%   relationships between the systems trigger cascading effects, which are
%   iteratively updated.
%
% INPUTS:
...
[PowerSysFunLoss, PowerComState, PowerZoneState] = SingleDCPF(PowerSystem, PowerComDamgScenario, PowerParams, TerminalZone);
...
CascadeTrace(1).PowerEdgeState=PowerComState.Edge(:,2);CascadeTrace(1).GasEdgeState=GasComState.Edge(:,2);
...
while (powerCascadeFlag || gasCascadeFlag )
    ...
    if (powerCascadeFlag || gasCascadeFlag)
        ...
        [PowerSysFunLoss, PowerComState, PowerZoneState] = SingleDCPF(PowerSystem, newPowerComDamgScenario, PowerParams, TerminalZone);
        ...
        CascadeTrace(tag).PowerEdgeState=PowerComState.Edge(:,2);CascadeTrace(tag).GasEdgeState=GasComState.Edge(:,2);
