function [ResLoss, SysFunsEvo, RepairSeq, ZoneStateEvo] = ComIndexOptSingleMF(CIS, CISComDamgScenario, TerminalZone, params)
% INTRODUCTION:
...
%   evaluates system functionality via a max flow model. Each
...
% operation variablkes+x(n,t)+eta(k,t)+r(k,n,t)+y(k,n,t)+alpha(k,n,t)+beta(k,n,t)+z(l,t)
Nv = (Ns+Nd+E)*(Tp-1) + Ndm*Tp + RR*Tp + Ndm*RR*Tp + Ndm*RR*Tp + Ndm*RR*(Tp-1) + Ndm*RR*(Tp-1) + E*(Tp-1);
...
Aeq1 = zeros(N*(Tp-1),Nv); beq1 = zeros(N*(Tp-1),1); etag1 = 0;
...
for tp = 1 : Tp-1
    ...
    for n = 1 : N
        ...
        etag1=etag1+1;beq1(etag1)=CIS.Node(n).TargetDemand;
        ...
        if CIS.Node(n).TargetDemand~=0 && CIS.Node(n).MaxGeneration~=0
            Aeq1(etag1,tag1+stag)=1;Aeq1(etag1,tag1+Ns+Nd+temp1)=-1;
            Aeq1(etag1,tag1+Ns+Nd+temp2)=1;Aeq1(etag1,tag1+Ns+dtag)=1;
        end
    end
...
Aeq2 = zeros(Tp,Nv); beq2 = zeros(Tp,1); 
Aeq3 = zeros(RR*Tp,Nv); beq3 = zeros(RR*Tp,1);rtag=0; 
Aeq4 = zeros(Ndm*Tp,Nv); beq4 = zeros(Ndm*Tp,1);etag5=0;
...
for tp=1:Tp
    ...
    for k = 1:RR
        rtag=rtag+1; Aeq3(rtag,tag3+k)=1;Aeq3(rtag,tag4+Ndm*(k-1)+(1:Ndm))=-1;beq3(rtag)=0;
    end

    % constraints for the state of each component
    for n = 1:Ndm
        etag5 = etag5+1;
        Aeq4(etag5,tag2+n) = 1; beq4(etag5) = 0;
        for s = 1:tp
            for k = 1 : RR
                tag = (Ns+Nd+E)*(Tp-1)+Ndm*Tp+RR*Tp+Ndm*RR*(s-1)+Ndm*(k-1);
                Aeq4(etag5,tag+n) = -1; % r(k,n,s)
            end
        end
    end
...
% 2) Build functionality curve and zone state evolution
SysFunsEvo(1,1) = 0;
