function [ResLoss, SysFunsEvo, RepairSeq, ZoneStateEvo] = HeuristicRepairSingleConnectivity(CIS, CISComDamgScenario, TerminalZone, params)
% INTRODUCTION:
...
% by a connectivity-based metric {'Pop','SDC','LCS','NPC'}.
%
% INPUT:
%   CIS                  : Struct of a single system with fields:
%                          - Node(1..N): array of nodes with at least IDs and coordinates
%                            Optionally: Node(k).MaxGeneration (for sources) 
%                          - Edge(1..E): array with fields FromNodeID, ToNodeID
%   CISComDamgScenario   : [Ndm x 4] matrix, each row = [ComponentType, ComponentID, RepairTime, SysType]
...
% Metric set (road typically has no SDC)
if ismember(lower(params.SystemType), {'power','gas','water'})
    allowedFunMetrics = {'Pop','SDC','LCS','NPC'};
else
...
if Ndm == 0
    switch params.FunMetric
        case 'Pop', sysfunc = @SinglePopConnectivity;
        case 'SDC', sysfunc = @SingleSDConnectivity;
        case 'LCS', sysfunc = @SingleLCSConnectivity;
        case 'NPC', sysfunc = @SingleNPConnectivity;
    end
    ...
    RepairSeq = [];
    ResLoss = [0, 0, 0];
    ZoneStateEvo=[(1:size(TerminalZone,2))',baseZoneState(:,1)];
...
switch params.ScheMethod
    case 'GA'
        [ResLoss, SysFunsEvo, RepairSeq, ZoneStateEvo] = GABasedRepairSchedulingSingleConnectivity(CIS, CISComDamgScenario, TerminalZone, params);
    case 'SA'
        [ResLoss, SysFunsEvo, RepairSeq, ZoneStateEvo] = SABasedRepairSchedulingSingleConnectivity(CIS, CISComDamgScenario, TerminalZone, params);
end
end


function [ResLoss, SysFunsEvo, RepairSeq, ZoneStateEvo] = GABasedRepairSchedulingSingleConnectivity(CIS, CISComDamgScenario, TerminalZone, params)
% INTRODUCTION:
...
% 
% INPUT:
% CIS, CISComDamgScenario,TerminalZone£¬ params : Same as main function. GA fields used¡£
%
% OUTPUT:
...
if Ndm <= RR 
    ...
    for i = 1:params.GANumInd
        ...
        [~, resloss] = computeRestorationCurveSingleConnectivity(CIS,repairseq,CISComDamgScenario, TerminalZone,params);
        ObjVC(i,1) = resloss(1);
    end
...
[SysFunsEvo, ResLoss, ZoneStateEvo] = computeRestorationCurveSingleConnectivity(CIS,RepairSeq,CISComDamgScenario, TerminalZone,params);
end


function [ResLoss, SysFunsEvo, RepairSeq, ZoneStateEvo] = SABasedRepairSchedulingSingleConnectivity(CIS, CISComDamgScenario, TerminalZone, params)
% INTRODUCTION:
...
function [SysFunsEvo, ResLoss, ZoneStateEvo] = computeRestorationCurveSingleConnectivity(CIS, RepairSeq, CISComDamgScenario, TerminalZone, params)
...
% Select functionality function handle
switch params.FunMetric
    case 'Pop', sysfunc = @SinglePopConnectivity;
    case 'SDC', sysfunc = @SingleSDConnectivity;
    case 'LCS', sysfunc = @SingleLCSConnectivity;
