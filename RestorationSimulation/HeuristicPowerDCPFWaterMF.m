function [PowerResLoss, WaterResLoss, PowerSysFunsEvo, WaterSysFunsEvo, PowerRepairSeq, WaterRepairSeq, PowerZoneStateEvo, WaterZoneStateEvo] ...
    = HeuristicPowerDCPFWaterMF (PowerSystem, WaterSystem, PowerWaterInterdependency, PowerComDamgScenario, WaterComDamgScenario, TerminalZone, params)
% INTRODUCTION:
%   Interdependent repair scheduling for power¨Cwater via GA/SA. Builds
...
%   (for power) and max-flow models (for water).
%
% INPUT:
...
%                         for power and water respectively.
%       .ScheMethod   'GA' or 'SA'
...
%   PowerRepairSeq, WaterRepairSeq:
%       Each is a subset of the integrated repair schedule:
%           [DamageType, ComponentID, SysType, FinishTime, TeamID]
%       filtered by SysType = 1 (power), or 3 (water) respectively.
%   PowerZoneStateEvo, WaterZoneStateEvo:
...
Scenarios = {PowerComDamgScenario, WaterComDamgScenario};
Names     = {'PowerComDamgScenario','WaterComDamgScenario'};
for i = 1:2
    S = Scenarios{i};
...
if isempty([PowerComDamgScenario;WaterComDamgScenario])
    PowerResLoss = [0, 0, 0];WaterResLoss = [0, 0, 0];
    PowerRepairSeq = [];WaterRepairSeq = [];
    [pLoss, wLoss,~,~,zonePower,zoneWater] = GlobalOptPowerDCPFWaterMF(PowerSystem, WaterSystem, PowerWaterInterdependency,...
        PowerComDamgScenario, WaterComDamgScenario, TerminalZone, params);
    PowerSysFunsEvo = [0, pLoss]; WaterSysFunsEvo = [0, wLoss];
    PowerZoneStateEvo=zonePower; WaterZoneStateEvo=zoneWater;
...
switch params.ScheMethod
    case 'GA'
        [PowerResLoss, WaterResLoss, PowerSysFunsEvo, WaterSysFunsEvo, PowerRepairSeq, WaterRepairSeq, PowerZoneStateEvo, WaterZoneStateEvo] ...
            = GABasedRepairSchedulingPowerDCPFWaterMF (PowerSystem, WaterSystem, PowerWaterInterdependency, PowerComDamgScenario, WaterComDamgScenario, TerminalZone,  params);
    case 'SA'
        [PowerResLoss, WaterResLoss, PowerSysFunsEvo, WaterSysFunsEvo, PowerRepairSeq, WaterRepairSeq, PowerZoneStateEvo, WaterZoneStateEvo] ...
            = SABasedRepairSchedulingPowerDCPFWaterMF (PowerSystem, WaterSystem,PowerWaterInterdependency, PowerComDamgScenario, WaterComDamgScenario, TerminalZone, params);
end
end
...
% PowerComDamgScenario, WaterComDamgScenario, params : Same as main
% function. GA fields used¡£
%
% OUTPUT:
% PowerResLoss, WaterResLoss, PowerSysFunsEvo, WaterSysFunsEvo,
% PowerRepairSeq, WaterRepairSeq : Same definitions as main function.

RR_power = params.RepairCrew(1);
RR_water = params.RepairCrew(2);

Ndm_power = size(PowerComDamgScenario,1);
Ndm_water = size(WaterComDamgScenario,1);
Ndm = Ndm_power + Ndm_water;
Integrated_ComDamgScenario = [PowerComDamgScenario; WaterComDamgScenario];

if Ndm_power <= RR_power && Ndm_water <= RR_water
    ...
    for i = 1:params.GANumInd
        ...
        repairseq = makeIntegratedSchedule(res_seq, PowerComDamgScenario, WaterComDamgScenario, RR_power, RR_water);
        [~, ~, pLoss, wLoss] = computeRestorationCurvePowerDCPFWaterMF(repairseq, PowerSystem, WaterSystem, ...
            PowerWaterInterdependency,PowerComDamgScenario, WaterComDamgScenario, TerminalZone);
        ObjVC(i,1) = pLoss(1)+wLoss(1);
    end
...
repairseq = makeIntegratedSchedule(opt_res_seq, PowerComDamgScenario, WaterComDamgScenario, RR_power, RR_water);
% Split by system for outputs
PowerRepairSeq = repairseq(repairseq(:,3)==1,:);
WaterRepairSeq = repairseq(repairseq(:,3)==3,:);
[PowerSysFunsEvo, WaterSysFunsEvo, PowerResLoss, WaterResLoss, PowerZoneStateEvo, WaterZoneStateEvo] = computeRestorationCurvePowerDCPFWaterMF...
    (repairseq, PowerSystem, WaterSystem,PowerWaterInterdependency,PowerComDamgScenario, WaterComDamgScenario, TerminalZone);
end
...
function repairseq = makeIntegratedSchedule(res_seq, PowerComDamgScenario, WaterComDamgScenario, RR_power, RR_water)
% INTRODUCTION:
...
repairseq = [repairseq_power; repairseq_water];
repairseq = sortrows(repairseq,4);
end


function [PowerSysFunsEvo, WaterSysFunsEvo, PowerResLoss, WaterResLoss, PowerZoneStateEvo, WaterZoneStateEvo] = computeRestorationCurvePowerDCPFWaterMF...
    (Integrated_schedule, PowerSystem, WaterSystem, PowerWaterInterdependency,PowerComDamgScenario, WaterComDamgScenario, TerminalZone)

%   Track functionality over an integrated (merged) repair timeline and
%   compute per-system resilience loss for power/water.
%
% INPUT:
%    Integrated_schedule: [K x 5] = [DamageType, ComponentID, SysType(1/2/3), FinishTime, TeamID]
%    PowerComDamgScenario/WaterComDamgScenario: [Ndm x 4]
% OUTPUT:
%    Power/WaterSysFunsEvo: [(K+1) x 4] with time in col1 shared by all systems
%    Power/WaterResLoss: [1 x 3] = [NormLoss, RealRes, ExpRes]
...
% Damage sets
Damg = {PowerComDamgScenario; []; WaterComDamgScenario};
params=struct();
[pLoss, wLoss,~,~,zonePower,zoneWater] = GlobalOptPowerDCPFWaterMF(PowerSystem, WaterSystem,  ...
    PowerWaterInterdependency, Damg{1}, Damg{3}, TerminalZone, params);
...
for k = 1:K
    sysID = Integrated_schedule(k,3);            % 1=power, 3=water
