function [ResLoss, SysFunsEvo, RepairSeq, ZoneStateEvo] = TimeIndexOptSingleDCPF(CIS, CISComDamgScenario, TerminalZone, params)
% INTRODUCTION:
%   This function formulates a time-indexed mixed-integer program to schedule
%   repairs of damaged components under a given damage scenario and evaluates
%   system functionality via a DC power flow (DCPF) model. The time horizon is
%   discretized into equally spaced indices; at each time index, repair decisions
%   are made and system functionality is computed.
%
%
% INPUTS:
...
% Vector length:
Nv = (Ns+Nd+E)*(Tp-1) + Ndm*Tp + Ndm*RR*Tp + E*(Tp-1) + N*(Tp-1);
f  = zeros(Nv,1);
lb = -inf(Nv,1);
ub = inf(Nv,1);

%% Step 3: Build MILP constraints
Aineq1 = zeros(E*4*(Tp-1),Nv);bineq1 = zeros(E*4*(Tp-1),1);  % Flow constrrains
...
for tp = 1: Tp-1
    ...
    for e = 1 : E
        ...
        Aineq1(inetag1,tag5+CIS.Edge(e).FromNodeID)=-CIS.Edge(e).Susceptance;
        Aineq1(inetag1,tag5+CIS.Edge(e).ToNodeID)=CIS.Edge(e).Susceptance;
        Aineq1(inetag1,tag4+e)=M;
        bineq1(inetag1)=M;
        
        Aineq1(inetag1+E,tag1+Ns+Nd+e)=-1;
        Aineq1(inetag1+E,tag5+CIS.Edge(e).FromNodeID)=CIS.Edge(e).Susceptance;
        Aineq1(inetag1+E,tag5+CIS.Edge(e).ToNodeID)=-CIS.Edge(e).Susceptance;
        Aineq1(inetag1+E,tag4+e)=M;
        bineq1(inetag1+E)=M;
        
        Aineq1(inetag1+2*E,tag1+Ns+Nd+e)=1;
        Aineq1(inetag1+2*E,tag4+e)=-CIS.Edge(e).Capacity;
        bineq1(inetag1+2*E)=0;
        
        Aineq1(inetag1+3*E,tag1+Ns+Nd+e)=-1;
        Aineq1(inetag1+3*E,tag4+e)=-CIS.Edge(e).Capacity;
    ...
    for n = 1 : N
        ...
        if CIS.Node(n).TargetDemand~=0 && CIS.Node(n).MaxGeneration~=0
            Aeq2(etag2,tag1+stag)=1;Aeq2(etag2,tag1+Ns+Nd+temp2)=1;
            Aeq2(etag2,tag1+Ns+Nd+temp1)=-1;Aeq2(etag2,tag1+Ns+dtag)=-1;
        end
    end
end
