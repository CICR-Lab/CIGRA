function [AttackStrategy, AttackCenter, PowerSysFunLoss, WaterSysFunLoss, PowerComState,  WaterComState] = ...
          LcAttackerRobPowerMFWaterMFOperator(PowerSystem, WaterSystem, PowerWaterInterdependency, ComSet, params, AttackPara, TerminalZone)
% INTRODUCTION:
%   Searches for the component set that maximises the combined
%   functionality loss of interdependent power and water systems based on MF model.
%   Each candidate in ComSet is evaluated with GlobalOptRobMFPowerMFWaterModel.
%
% INPUT:
%   PowerSystem, waterSystem  – CIS-style structs (see GlobalOptRobMFPowerMFwaterModel).
...
%   params                  – passed to GlobalOptRobMFPowerMFwaterModel.
...
%   PowerSysFunLoss, WaterSysFunLoss – 1×3 vectors from GlobalOptRobMFPowerMFwaterModel.
...
assert(numel(InvulnerableCom) == TotCom, ...
      'InvulnerableCom length mismatch (expected %d).');
...
for r = 1:size(ComSet,1)
    ...
    [Ploss, Wloss, Pstate, Wstate] = GlobalOptPowerMFWaterMF( ...
