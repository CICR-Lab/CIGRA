function [AttackStrategy, AttackCenter, PowerSysFunLoss, GasSysFunLoss, PowerComState, GasComState] = ...
          LcAttackerRobPowerMFGasMFOperator(PowerSystem, GasSystem, PowerGasInterdependency, ComSet, params, AttackPara, TerminalZone)
% INTRODUCTION:
%   Searches for the component set that maximises the combined
%   functionality loss of interdependent power and gas systems based on MF model.
%   Each candidate in ComSet is evaluated with GlobalOptRobMFPowerMFGasModel.
%
% INPUT:
%   PowerSystem, GasSystem  – CIS-style structs (see GlobalOptRobMFPowerMFGasModel).
...
%   params                  – passed to GlobalOptRobMFPowerMFGasModel.
...
%   PowerSysFunLoss, GasSysFunLoss – 1×3 vectors from GlobalOptRobMFPowerMFGasModel.
...
for c = 1:size(ComSet,1)
    ...
    [P_loss, G_loss, P_state, G_state] = GlobalOptPowerMFGasMF( ...
