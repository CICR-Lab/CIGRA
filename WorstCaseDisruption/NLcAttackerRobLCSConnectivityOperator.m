function [AttackStrategy, SysFunLoss, ComState] = NLcAttackerRobLCSConnectivityOperator(CIS, AttackParams, OperatorParams)
% AttackerRobLCSConnectivityBasedOperatorNPA determines an optimal attack strategy on a critical infrastructure system
% (CIS) such as PowerSystem, GasSystem, or WaterSystem, aiming to minimize the size of the largest connected component
% (LCS) in the system within a given attack budget. The function uses a mixed-integer linear programming (MILP) approach
% to select nodes and edges to attack, considering node and edge attack costs, and evaluates the resulting system
% performance based on the size of the largest connected component.
%
% Inputs:
...
%             - Column 1: Size of the connected component containing each node post-attack (0 if node is attacked).
%             - Column 2: Size of the largest connected component pre-attack.
...
% Define attack decision variables
...
for i = 1:N
    for j = (i+1):N
        f(N + E + node_pair_net(i, j)) = 1; % Coefficient of 1 to maximize node pair disconnections (MILP minimizes, so this maximizes disruption)
    end
end

% Compute initial connected components
G_original = graph(eid_net); % Create graph object from original adjacency matrix
components_original = conncomp(G_original); % Compute connected components
component_sizes_original = histcounts(components_original, 1:max(components_original)+1); % Size of each component
maxComponentSize_original = max(component_sizes_original); % Size of the largest connected component
...
% Compute connected components in the residual network
G_residual = graph(residual_net); % Create residual graph
components = conncomp(G_residual); % Compute connected components
component_sizes = histcounts(components, 1:max(components)+1); % Size of each component
maxComponentSize = max([component_sizes, 0]); % Size of the largest connected component post-attack (handle empty case)

% Compute connectivity state for each node
ComState = zeros(N, 2); % Initialize connectivity state matrix
for n = 1:N
    ComState(n, 2) = maxComponentSize_original; % Pre-disaster: size of largest connected component
    if ~ismember(n, attacked_nodes)
        ComState(n, 1) = sum(components == components(n)); % Post-disaster: size of component containing node n
    else
        ComState(n, 1) = 0; % Attacked nodes have no connectivity
    end
end

% Compute system functionality loss
SysFunLoss = 1 - [maxComponentSize/N ... % Normalized functionality loss (post-attack LCS size / total nodes)
                  maxComponentSize/N ... % Post-disaster functionality (weighted average component size)
                  maxComponentSize_original/N]; % Pre-disaster functionality (original LCS size / total nodes)
end