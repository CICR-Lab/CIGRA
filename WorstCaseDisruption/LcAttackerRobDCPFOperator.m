function [AttackStrategy, AttackCenter, SysFunLoss, ComState] = LcAttackerRobDCPFOperator(CIS, ComSet, params, AttackPara, TerminalZone)
% INTRODUCTION:
% This function simulates an attacker seeking the worst-case 
% damage scenario bease DCPF model that causes the largest drop in single system functionality.
...
%
%   params – struct used in `SingleDCPFModel` for functionality evaluation (same fields as in `SingleMFModel`)
%
...
for nc = 1:size(ComSet, 1)
    ...
    [SysFunLossNow, ComStateNow] = SingleDCPF(CIS, CISComDamgScenario, params, TerminalZone);
