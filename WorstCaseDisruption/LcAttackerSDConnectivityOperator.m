function [AttackStrategy, AttackCenter, SysFunLoss, ComState] = LcAttackerSDConnectivityOperator(CIS, ComSet, params, AttackPara, TerminalZone)
% INTRODUCTION:
% This function simulates an attacker seeking the worst-case 
% damage scenario based on connectivity between source node and demand node that causes the largest drop in node-level functionality.
% It evaluates each predefined damage combination (component set) and selects 
% the one that leads to maximum loss of the system functionality.
%
% INPUT:
...
for nc = 1:size(ComSet, 1)
    ...
    [SysFunLossNow, ComStateNow] = SingleSDConnectivity(CIS, CISComDamgScenario, params, TerminalZone);
