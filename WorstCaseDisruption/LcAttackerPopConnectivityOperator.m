function [AttackStrategy, AttackCenter, SysFunLoss, ComState] = LcAttackerPopConnectivityOperator(CIS, ComSet, params, AttackPara, TerminalZone)
% INTRODUCTION:
% This function simulates an attacker seeking the worst-case 
% damage scenario based on served population that causes the largest drop in node-level functionality.
% It evaluates each predefined damage combination (component set) and selects 
% the one that leads to maximum loss of served population.
%
% INPUT:
...
max_loss = -inf;  % Initialize maximum loss (用 -inf 更保险)
...
for nc = 1:size(ComSet, 1)
    ...
    npdm = temp(temp >= 1 & temp <= N);          % Node indices (1..N)
    epdm = temp(temp >= N+1 & temp <= N+E) - N;  % Edge indices (1..E) after -N conversion
    ...
    CISComDamgScenario = [];
    if ~isempty(npdm)
        CISComDamgScenario = [CISComDamgScenario; ones(length(npdm), 1), npdm(:)];
    end
    if ~isempty(epdm)
        CISComDamgScenario = [CISComDamgScenario; 2 * ones(length(epdm), 1), epdm(:)];
    end
    
    % Step 3: Evaluate functionality under current attack scenario
    [SysFunLossNow, ComStateNow] = SinglePopConnectivity(CIS, CISComDamgScenario, params, TerminalZone);
