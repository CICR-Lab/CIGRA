function [AttackStrategy, SysFunLoss, ComState] = NLcAttackerRobNPConnectivityOperator(CIS, AttackParams, OperatorParams)
% AttackerRobNPConnectivityBasedOperatorNPA determines an optimal attack strategy on a critical infrastructure system
% (CIS) such as PowerSystem, GasSystem, or WaterSystem, aiming to maximize node-pair-connectivity-based system functionality
% loss within a given attack budget. The function uses a mixed-integer linear programming (MILP) approach to select nodes
% and edges to attack, considering node and edge attack costs, and evaluates the resulting system performance based on
...
%             - Column 1: Post-disaster connectivity level (normalized by number of other nodes).
%             - Column 2: Pre-disaster connectivity level (normalized to 1).
...
vid = 0; % Counter for node pair indices
for i = 1:N
    temp_node_weight = node_weight; % Copy node weights
    temp_node_weight(i) = 0; % Exclude node i from weight sum
    for j = (i+1):N
        vid = vid + 1; % Increment node pair index
        f(N+E+vid) = node_weight(i) * node_weight(j) / (sum(node_weight) * sum(temp_node_weight)); % Weight for node pair (i,j)
    end
end

% Update objective function for symmetric node pairs
for j = 1:N
    temp_node_weight = node_weight; % Copy node weights
    temp_node_weight(j) = 0; % Exclude node j from weight sum
    for i = (j+1):N
        f(N+E+node_pair_net(j,i)) = f(N+E+node_pair_net(j,i)) + ...
            node_weight(j) * node_weight(i) / (sum(node_weight) * sum(temp_node_weight)); % Add weight for node pair (j,i)
    end
end
...
% intcon = 1:Nv; % 假设�? Nv 个变量是整数（在二进制情况下，整数即为二进制�?
% 
% % 定义求解器�?�项
% options = optimoptions('intlinprog', 'Display', 'off'); % 禁用显示
% 
% % 求解线�?�规划问�?
...
% Compute residual node-pair connectivity
rnode_fun = zeros(N, N); % Matrix to store post-attack node-pair connectivity
for i = 1:N
    for j = (i+1):N
        rnode_fun(i, j) = x(N+E+node_pair_net(i,j)); % Node pair connectivity from MILP solution
        rnode_fun(j, i) = rnode_fun(i, j); % Symmetric assignment
    end
end

% Compute system functionality for each node
for n = 1:N
    temp_node = setdiff((1:N)', n); % All nodes except n
    sysFun(n, 1) = sum(rnode_fun(temp_node, n) .* node_weight(temp_node)) / sum(node_weight(temp_node)); % Post-disaster functionality
    sysFun(n, 2) = sum((temp_node*0+1) .* node_weight(temp_node)) / sum(node_weight(temp_node)); % Pre-disaster functionality (normalized to 1)
end

% Compute connectivity state for each node
ComState = zeros(N, 2); % Initialize connectivity state matrix
for n = 1:N
    ComState(n, 1) = sum(rnode_fun(n, :)) / (N-1); % Post-disaster connectivity (normalized by number of other nodes)
    ComState(n, 2) = 1; % Pre-disaster connectivity (normalized to 1)
end
